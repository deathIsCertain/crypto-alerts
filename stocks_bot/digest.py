...
def pick_top3(articles):
    ...
    prompt = (
        "Below is a numbered list of today's Indian stock market headlines. Pick the 3 most "
        "impactful for an investor in Indian markets. Reply with exactly 3 lines, each in the format:\n"
...
def _build_weekly_pdf(archive):
    ...
    font = _ff()
    title_style = ParagraphStyle("Title", fontName=font, fontSize=16, leading=20, textColor=colors.HexColor("#1f9d55"))
...
def send_weekly_pdf(state_file):
    ...
    filename = f"stocks_weekly_{now.strftime('%Y-%m-%d')}.pdf"
    TelegramAlert().send_document(pdf_bytes, filename, f"India Stocks News — past 7 days ({len(archive)} articles)")
